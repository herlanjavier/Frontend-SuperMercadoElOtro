...
export default function SalesListPage() {
  const { sales, pagination, filters, setFilters, setPage, setLimit, isLoading, error, refetch } = useSalesList();
  ...
  return (
    <div className="grid gap-6">
      ...
      {!isLoading && !error && sales.length > 0 ? (
        <>
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{sales.map((sale) => <SaleCard key={sale.id} sale={sale} />)}</section>
