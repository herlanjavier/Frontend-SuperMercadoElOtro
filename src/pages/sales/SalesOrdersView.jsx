...
export default function SalesOrdersView({ title = 'Pedidos', subtitle = 'Gestiona los pedidos del supermercado', initialStatus = '' }) {
  const { orders, pagination, filters, setFilters, setPage, setLimit, isLoading, error, refetch } = useSalesOrders({ status: initialStatus });
  ...
  return (
    <div className="grid gap-6">
      ...
      {!isLoading && !error && orders.length > 0 ? (
        <>
          <SalesOrderTable orders={orders} onChanged={refetch} />
